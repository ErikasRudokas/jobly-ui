...
export const userService = {
  getMyProfile: async (): Promise<UserProfile> => {
    const response = await axiosInstance.get<UserProfile>(buildApiUrl(USER_ENDPOINTS.MY_PROFILE));
    return response.data;
  },

  getUserProfile: async (): Promise<GetUserProfileResponse> => {
    const response = await axiosInstance.get<GetUserProfileResponse>(buildApiUrl(USER_ENDPOINTS.PROFILE));
    return response.data;
  },
