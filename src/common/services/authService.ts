...
export const authService = {
  login: async (credentials: LoginRequest): Promise<void> => {
    const response = await axios.post<AuthResponse>(buildApiUrl(AUTH_ENDPOINTS.LOGIN), credentials);
  ...
  register: async (userData: RegisterRequest): Promise<void> => {
    await axios.post(buildApiUrl(AUTH_ENDPOINTS.REGISTER), userData);
  },
