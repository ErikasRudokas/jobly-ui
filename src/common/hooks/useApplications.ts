...
export const useApplications = () => {
  ...
  const getMyApplicationDetails = async (id: number): Promise<MyApplication | null> => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<MyApplication>(buildApiUrl(APPLICATION_ENDPOINTS.GET_MINE_DETAILS(id)));
      return response.data;
    } catch (err) {
  ...
  const updateApplication = async (id: number, data: ApplicationUpdateRequest): Promise<MyApplication | null> => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.put<MyApplication>(buildApiUrl(APPLICATION_ENDPOINTS.UPDATE(id)), data);
      return response.data;
    } catch (err) {
    ...
    try {
      ...
      if (err instanceof AxiosError) {
        if (err.response?.status === 400) {
          errorMessage =
            err.response?.data?.message || err.response?.data?.error || 'You have already applied to this job offer';
        } else {
          errorMessage =
            err.response?.data?.message || err.response?.data?.error || err.message || 'Failed to apply to job offer';
        }
      } else {
  ...
  const manageApplication = async (
    ...
    setError(null);
    try {
      await axiosInstance.post(buildApiUrl(APPLICATION_ENDPOINTS.MANAGE(id)), data);
      return { success: true };
    } catch (err) {
      let errorMessage: string;
      ...
      if (err instanceof AxiosError) {
        errorMessage =
          err.response?.data?.message || err.response?.data?.error || err.message || 'Failed to manage application';
      } else {
