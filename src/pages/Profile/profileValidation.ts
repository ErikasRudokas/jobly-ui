...
export const validateProfileData = (data: ValidationData): {
    isValid: boolean;
    ...
    try {
        profileFormSchema.parse({
            workExperience: data.workExperience.filter(we => !we.delete).map(we => ({
                ...we,
                id: we.isNew ? undefined : we.id,
            })),
            education: data.education.filter(e => !e.delete).map(e => ({
                ...e,
                id: e.isNew ? undefined : e.id,
            })),
            skills: data.skills.filter(s => !s.delete).map(s => ({
