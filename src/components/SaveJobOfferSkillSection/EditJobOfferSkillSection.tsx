...
const EditJobOfferSkillSection = ({
    ...
    const [skillsMetadata, setSkillsMetadata] = useState<Map<number, {name: string; type: 'TECHNICAL' | 'SOFT'}>>(new Map());
    ...
    const handleAddSkill = (skill: Skill) => {
        ...
        setSkillsMetadata(prev => new Map(prev).set(skill.id, {
            name: skill.name,
            type: skill.type
        }));
    ...
    const handleToggleDelete = (skillId: number) => {
        onSkillsChange(
            selectedSkills.map(s =>
                s.skillId === skillId ? {...s, delete: !s.delete} : s
            )
        );
    };
                ...
                {[1, 2, 3, 4].map(level => (
                    <Box
                        ...
                        sx={
                            isDeleted
                                ? proficiencyBarStyle
                                : (level <= currentLevel ? proficiencyBarFilledStyle : proficiencyBarStyle)
                        }
                        onClick={() =>
