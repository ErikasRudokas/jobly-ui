      ...
      <Box sx={proficiencyBarContainerStyle}>
        {[1, 2, 3, 4].map((level) => (
          <Box
            ...
            onClick={() => !disabled && handleProficiencyChange(skill.skillId, getProficiencyFromLevel(level))}
          />
        ))}
      </Box>
    );
                  ...
                  <Box key={skill.skillId} sx={skillItemStyle}>
                    <Box sx={skillHeaderStyle}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <Typography sx={skillProficiencyStyle}>{formatProficiency(skill.proficiency)}</Typography>
                        <IconButton
                          size="small"
