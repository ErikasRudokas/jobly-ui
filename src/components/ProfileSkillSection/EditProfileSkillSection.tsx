...
const EditProfileSkillSection = ({
  ...
  const handleRemoveSkill = (skillId: number) => {
    ...
    if (skill?.isNew) {
      ...
      onSkillsChange(skills.map((s) => (s.id === skillId ? { ...s, delete: !s.delete } : s)));
    }
  };

  const handleProficiencyChange = (skillId: number, proficiency: SkillProficiency) => {
    onSkillsChange(skills.map((skill) => (skill.id === skillId ? { ...skill, proficiencyLevel: proficiency } : skill)));
  };
      ...
      <Box key={skill.id} sx={isMarkedForDeletion ? deletedSkillItemStyle : skillItemStyle}>
        <Box sx={skillHeaderStyle}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {!isMarkedForDeletion && skill.isNew && <Typography sx={statusTextStyle}>Added</Typography>}
  ...
  const excludedSkillIds = skills.filter((s) => !s.delete).map((s) => s.skill.id);

  return (
    <Box sx={containerStyle}>
      <SkillSearchBox excludeSkillIds={excludedSkillIds} onAddSkill={handleAddSkill} disabled={disabled} />
