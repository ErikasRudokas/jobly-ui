    ...
    return (
        <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem'}}>
            <Box sx={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                {!isMarkedForDeletion && isNew && (
                    <Typography sx={statusBadgeStyle}>Added</Typography>
                )}
                {!isMarkedForDeletion && !isNew && isModified && (
                    <Typography sx={statusBadgeStyle}>Modified</Typography>
                )}
            </Box>
            <Box sx={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                {!isNew && (
